    ...
    <div className="w-full flex flex-col">
      ...
      <div className="h-[300px] flex flex-col py-2 gap-2 overflow-y-scroll scrollbar-thin scrollbar-thumb-bapgray scrollbar-track-bapgraylight">
        {filteredAnnouncements.map((announcement) => (
          <div 
            key={announcement.id} 
            className={`w-full border border-bapgray rounded-md px-4 py-2 flex flex-col bg-white relative ${announcement.is_pinned ? 'border-bapred bg-red-50' : ''}`}
          >
            <div className="flex justify-between items-start mb-1">
              <div className="flex items-center">
                <h3 className="font-semibold text-gray-800 text-m pr-2">{announcement.title}</h3>
