...
export default function AdminMemberEditModal({
  ...
  const [formData, setFormData] = useState<MemberDetail | null>(
    profileData
      ? { ...profileData, role: profileData.role || "general-member" }
      : null
  );
  ...
  const handleSave = async (updatedData: MemberDetail) => {
    try {
      const {
        data: { session },
      ...
      if (
        updatedData.role &&
        (!profileData || updatedData.role !== profileData.role)
      ) {
