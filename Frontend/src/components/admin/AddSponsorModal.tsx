...
interface AddSponsorModalProps {
    onClose: () => void;
    onSponsorAdded: (newSponsor: any) => void;
}
        ...
        <>
            ...
            >
                ...
                >
                    ...
                    <h2 className="text-xl font-semibold text-bapred mb-5 text-center">Add New Sponsor</h2>
                    ...
                    <div className="mb-4">
                        ...
                        <input
                            ...
                            value={sponsor}
                            onChange={(e) => { setSponsor(e.target.value); clearError('sponsor'); }}
                            className={`w-full border rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-bapred ${errors.sponsor ? 'border-red-500' : 'border-gray-300'}`}
                            required
                            aria-invalid={!!errors.sponsor}
                            aria-describedby={errors.sponsor ? "sponsor-error" : undefined}
                        />
                        {errors.sponsor && <p id="sponsor-error" className="text-red-500 text-xs mt-1">{errors.sponsor}</p>}
                    </div>

                    {/* Email List */}
                    <div className="mb-4">
                        <label htmlFor="emailList" className="block text-sm font-medium text-gray-700 mb-1">Email List *</label>
                        <textarea
                            id="emailList"
                            placeholder="Enter email addresses separated by commas..."
                            value={emailList}
                            onChange={(e) => { setEmailList(e.target.value.split(',').map(email => email.trim())); clearError('emailList'); }}
                            className={`w-full border rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-bapred ${errors.emailList ? 'border-red-500' : 'border-gray-300'}`}
                            rows={3}
                            required
                            aria-invalid={!!errors.emailList}
                            aria-describedby={errors.emailList ? "emailList-error" : undefined}
                        />
                        {errors.emailList && <p id="emailList-error" className="text-red-500 text-xs mt-1">{errors.emailList}</p>}
                    </div>

                    {/* Passcode */}
                    <div className="mb-4">
                        <label htmlFor="passcode" className="block text-sm font-medium text-gray-700 mb-1">Passcode *</label>
                        <input
                            id="passcode"
                            type="text"
                            placeholder="Enter passcode..."
                            value={passcode}
                            onChange={(e) => { setPasscode(e.target.value); clearError('passcode'); }}
                            className={`w-full border rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-bapred ${errors.passcode ? 'border-red-500' : 'border-gray-300'}`}
                            required
                            aria-invalid={!!errors.passcode}
                            aria-describedby={errors.passcode ? "passcode-error" : undefined}
                        />
                        {errors.passcode && <p id="passcode-error" className="text-red-500 text-xs mt-1">{errors.passcode}</p>}
                    </div>                    
